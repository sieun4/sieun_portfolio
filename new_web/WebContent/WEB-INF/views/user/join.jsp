		...
		$.ajax({ // ajax라는 함수 호출. {}안은 전부 매개변수
			...
			success : function(result, textStatus, jqXHR) { // 콜백 함수
				...
				if (result == 0) { // 중복 아님
					...
					alert("ID가 중복됩니다."); // ID가 중복됩니다. 라는 경고창
		...
		<form name="joinForm">
			<div class="form-item">
				<label for="userId"></label> <input type="text" name="userId"
					id="j_userId" required="required" placeholder="아이디  (필수)"/>
			</div>
			<div class="form-item">
				<label for="username"></label> <input type="text" placeholder="이름  (필수)"
					name="userName" id="j_userName" required="required" />
			</div>
			<div class="form-item">
				<label for="email"></label> <input type="text"
					name="nickname" id="j_nickname" required="required" placeholder="별명  (필수)"/>
			</div>
			<div class="form-item">
			...
			<div class="form-item">
				<label for="cpassword"></label> <input placeholder="비밀번호 재확인  (필수)" id="j_cUserPw"
					type="password" name="cPassword" id="j_cUserPw" required="required" />
			</div>
			<div class="form-item">
			...
			<div class="button-panel">
				<input class="button" onclick="doJoin()" title="Sign In" value="가입하기"></input>
			</div>
		</form>
